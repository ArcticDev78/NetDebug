...
print(
    yellow(
        " => dns-check - speedtest - router-check - ping - show [choice] - assistant",
        "bold",
    )
)  # noqa
...
print(
    blue(
        ' => "Having network issues? This tool could help you solve them!"',
        ["bold", "italic"],
    )
)  # noqa
...
table = [
    ["IP Address", "MAC Address", "Gateway", "Iface", "SSID"],
    # ["", "", "", "", ""],
...
print()
print(
    f'{green("[+] Please type", "bold")} {yellow("help", "bold")} {green("to view commands", "bold")}'
)  # noqa
print()
...
def dns_check():
    print(
        f'{yellow("netdebug", "underlined")} => {blue("(dns-check)", "bold")}\n'
    )  # noqa
    print(
        f'{green("Available DNS servers:", "underlined")}\n{cyan("1", "bold")} - Cloudflare DNS (1.1.1.1) [Recommended]\n{cyan("2", "bold")} - Google DNS (8.8.8.8)\n{cyan("3", "bold")} - Other (enter server address)\n'
    )  # noqa
    dns_server_choice = custom_input(
        "Which DNS server would you like to use? (1/2/3): "
    )  # noqa
...
╘══════════════════════════════════════════════════╛     \033[1;m"""
            ...
            table = [
                ["Minimum Ping", "Average Ping", "Maximum Ping"],
                [f"{min_ping} ms", f"{average_ping} ms", f"{max_ping} ms"],
            ]  # noqa
            print(
                tabulate(
...
╘═════════════════════════════════════════════════════════════╛     \033[1;m"""
                ...
                table = [
                    ...
                    [
                        f'Your (average) ping is {green("NORMAL", ["bold", "underlined"])}',
                        f'Average ping is {yellow("less <= 30", "bold")}',
                    ],
                ]  # noqa
...
╘═════════════════════════════════════════════════════════════╛     \033[1;m"""
                ...
                table = [
                    ["Status", "Condition met"],
                    [
                        f'Your (average) ping is {green("OK", ["bold", "underlined"])}',
                        f'Average ping is {yellow("> 30 and <= 100", "bold")}',
                    ],
                ]  # noqa
...
╘═════════════════════════════════════════════════════════════╛     \033[1;m"""
                ...
                table = [
                    ["Status", "Condition met"],
                    [
                        f'Your (average) ping is {red("HIGH", ["bold", "underlined"])}',
                        f'Average ping is {yellow("> 100 and <= 200", "bold")}',
                    ],
                ]  # noqa
...
╘═════════════════════════════════════════════════════════════╛     \033[1;m"""
                ...
                table = [
                    ["Status", "Condition met"],
                    [
                        f'Your (average) ping is {red("VERY HIGH", ["bold", "underlined"])}',
                        f'Average ping is {yellow("> 200 and <= 500", "bold")}',
                    ],
                ]  # noqa
            ...
            print()
            error_message(
                "Unable to reach DNS server. This could be because of the following reasons:\n1. The DNS server is down.\n2. Please check your internet connection if it is working."
            )  # noqa
            prompt()
...
╘═════════════════════════════════════════════════════════════╛     \033[1;m"""
                ...
                print()
                info_message(
                    "Your (average) ping is OK. [Average ping is >= 100]"
                )  # noqa
            elif average_ping < 100 and average_ping >= 200:
                print()
                info_message(
                    "Your (average) ping is high. [Average ping is >= 200]"
                )  # noqa
            elif average_ping < 200 and average_ping >= 500:
                print()
                info_message(
                    "Your (average) ping is VERY HIGH. [Average ping is >= 500"
                )  # noqa
            ...
            print()
            error_message(
                "Unable to reach DNS server. Please check your internet connection."
            )  # noqa
            print()
        ...
        print()
        custom_dns_server = custom_input(
            "Enter the address of the DNS server you would like to use: "
        )  # noqa
        ...
        if dot_count < 3:
            print()
            error_message(
                "The DNS server that you entered is not valid.",
                "The DNS server address must contain at least 3 dots. Example: 1.1.1.1\n",
            )  # noqa
            prompt()
        elif dot_count == 3:
            print()
            info_message(
                f"Running DNS Check using Custom DNS server ({custom_dns_server})...\n"
            )  # noqa
...
╘══════════════════════════════════════════════════╛     \033[1;m"""
                ...
                lastLine = os.popen(
                    f"ping {custom_dns_server} -c 5 | tail -n 1"
                ).read()  # noqa
                lastLine2 = lastLine.replace("rtt ", "")
                ping_ar = lastLine2.split()
...
╘═════════════════════════════════════════════════════════════╛     \033[1;m"""
        ...
        print()
        error_message(
            f'Invalid option: "{dns_server_choice}". Please enter a valid choice.'
        )  # noqa
        print()
        prompt()
...
def speedtest():
    ...
    if response == 0:
        ...
        print()
        error_message(
            "Unable to reach Ookla speedtest servers. Please check your internet connection.\n"
        )  # noqa
    prompt()
...
def show_ip():
    print(
        f'{cyan("1", "bold")} - Public IP address\n{cyan("2", "bold")} - Private IP address\n'
    )  # noqa
    ...
    if choice == "1":
        try:
            print()
            info_message(
                f'Public IP address: {requests.get("http://ipecho.net/plain?").text}\n'
            )  # noqa
        except Exception:
            print()
            error_message(
        ...
        if priv_ip == "\n":
            print()
            error_message(
                "Error - Unable to get private IP address. Please check your connection.\n"
            )  # noqa
        else:
            print()
...
def show_mac():
    ...
    if n_mac == "":
        error_message(
            "Error - Unable to get MAC address. Please check your connection.\n"
        )  # noqa
    else:
...
def show_ssid():
    ...
    if n_name == "":
        error_message(
            "Error - Unable to get SSID. Please check if you are connected to a network.\n"
        )  # noqa
    else:
...
def show_gateway():
    ...
    if gateway == "":
        error_message(
            "Error - Unable to get gateway. Please check your connection.\n"
        )  # noqa
    else:
...
╘═════════════════════════════════════════════════════════════════════════╛     \033[1;m"""
        ...
        print(
            f'{green("Available network data to show:", "underlined")}\n{cyan("1", "bold")} - Private/Public IP Address\n{cyan("2", "bold")} - MAC Address\n{cyan("3", "bold")} - SSID (Network name)\n{cyan("4", "bold")} - Gateway\n{cyan("all", "bold")} - Show all network data\n'
        )  # noqa
        data_choice = custom_input(
            "Which network data would you like to see? (1/2/3/4/all): "
        )  # noqa
        if data_choice == "1":
            print()
            show_ip()
...
def router_check():
    print(
        f'{yellow("netdebug", "underlined")} => {blue("(router-check)", "bold")}\n'
    )  # noqa
    gateway = os.popen(
        "ip route show | grep -i 'default via'| awk '{print $3 }'"
    ).read()  # noqa
...
def assistant():
    ...
    print(
        f'{yellow("netdebug", "underlined")} => {blue("(assistant)", "bold")}\n'
    )  # noqa
    # custom_input('ASSISTANT: What do you need help with?')
    print(f'{cyan("ASSISTANT:", "bold")} Hello, what do you need help with?\n')
    print(
        f'  {yellow("1. What is my internet (upload/download) speed?", "bold")}\n  ↳ {green("Runs speedtest", "italic")}'
    )  # noqa                                                                                                                               prompt()
    print(
        f'  {yellow("2. Show me my network info", "bold")}\n  ↳ {green("Shows network data of selection", "italic")}'
    )  # noqa
    print(
        f'  {yellow("3. What is my (DNS) ping/latency?", "bold")}\n  ↳ {green("Runs dns-check", "italic")}'
    )  # noqa
    print(
        f'  {yellow("4. Is my device able to reach my router?", "bold")}\n  ↳ {green("Runs a router check by pinging the gateway (router)", "italic")}'
    )  # noqaprint(f'  {yellow("exit", "bold")}            |  {green("exit the program", "italic")}')  # noqa""" PROMPT """
    print(
        f'  {yellow("5. I need to check if a website is down or up.", "bold")}\n  ↳ {green("Runs ping", "italic")}'
    )  # noqa
    ...
    user_choice = custom_input(
        "Which one would you like help with? (1/2/3/4/5): "
    )  # noqa
    if user_choice == "1":
        print()
        speedtest()
...
def prompt():
    prompt_input = input(
        yellow("netdebug", "underlined") + " " + green(">") + " "
    )  # noqa
        ...
        if command == "dns-check":
            ...
            try:
                if args[1] == "ip" or args[1] == "1":
                    ...
                    print()
                    error_message(
                        f'Invalid option: "{args[1]}". Please enter a valid choice.'
                    )  # noqa
                    print()
            ...
            print(
                f'  {yellow("show", "bold")} <choice>   |  {green("Show network data of choice (or select from menu)", "italic")}'
            )  # noqa
            print(
                f'  {yellow("dns-check", "bold")}       |  {green("Run a DNS check by pinging DNS servers", "italic")}'
            )  # noqa
            print(
                f'  {yellow("router-check", "bold")}    |  {green("Run a router check by pinging the gateway (router)", "italic")}'
            )  # noqa
            print(
                f'  {yellow("ping", "bold")}            |  {green("Run a ping check by pinging the specified address or IP", "italic")}'
            )  # noqa
            print(
                f'  {yellow("assistant", "bold")}       |  {green("Solve your network issues with the help of the NetDebug Assistant!", "italic")}'
            )  # noqa
            print(
                f'  {yellow("speedtest", "bold")}       |  {green("Run a network speed test using Ookla Speedtest", "italic")}'
            )  # noqa
            print(
            ...
            )  # noqaprint(f'  {yellow("exit", "bold")}            |  {green("exit the program", "italic")}')  # noqa
            print()
            prompt()
